...
"""Settings for the TrainInput and TrainTSInput
...
class TrainInput:
    ...
    Attributes:
        data(class:`DataUpload <Job>`): Train data
        train_body(class `TrainBody <CoreXOBJ>`):
    ...
    def __init__(
        ...
        if ts_split_train is None:
            ...
            train = CoreXOBJ.CVTrain.create(
                start=ts_split_train['start'], end=ts_split_train['end'])

        if ts_split_test is None:
            test = None
        else:
            test = CoreXOBJ.CVTrain.create(
                start=ts_split_test['start'], end=ts_split_test['end'])

        cv = CoreXOBJ.cv_obj_array(ts_split_cv)

        time_series_split = CoreXOBJ.TimeSeriesSplit.create(
            split_by=ts_split_split_by,
            cv=cv,
            train=train,
            test=test)

        self.train_body = CoreXOBJ.TrainBody.create(
            target=target,
    ...
    def get_train_params(self):
        ...
        params = json.dumps(
            self.train_body.jsonable(), cls=CoreXOBJ.ComplexEncoder)
...
class TrainTSInput:
    ...
    Attributes:
        data(class:`DataUpload <Job>`): Train data
        train_auto_ts_body(class `TrainTrainAutoTSBodyBody <CoreXOBJ>`):
    ...
    def __init__(
        ...
        self.data = data
        model_build_control = CoreXOBJ.ModelBuildControl.create(
            tolerance=tolerance,
            validation_percentage=validation_percentage,
            max_model=max_model)

        genetic_algorithm = CoreXOBJ.GeneticAlgorithmParams.create(
        ...
        build_control = CoreXOBJ.BuildControl.create(
        ...
        model_spec = CoreXOBJ.ModelSpec.create(
        ...
        feature_types = CoreXOBJ.column_array(feature_types)

        group_by = CoreXOBJ.TSGroupBy.create(
            time_unit=time_unit,
            regression_method=regression_method,
            classification_method=classification_method)

        input_spec = CoreXOBJ.InputSpec.create(
            train_data_id='tmp_data_id',
            target=target,
        ...
        self.train_auto_ts_body = CoreXOBJ.TrainAutoTSBody.create(
            callback=callback,
    ...
    def get_train_params(self):
        ...
        params = json.dumps(
            self.train_auto_ts_body.jsonable(), cls=CoreXOBJ.ComplexEncoder)
        params = json.loads(params)
        return params
