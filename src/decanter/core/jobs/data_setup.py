...
from decanter.core.jobs.task import SetupTask, GPSetupTask
...
class GPDataSetup(Job):
    """DataSetup manage to get the result from data setup.

    Handle the execution of setup task in order to setup data to
    Decanter GP server; Stores the setup results in DataSetup attributes.
    ...
    def create(cls, data_id, name=None):
        ...
        data = cls()
        data_resp = check_response(
            data.gp_service.get_table_by_id(data_id)).json()
        data.update_result(data_resp)
        data.status = CoreStatus.DONE
        data.name = name
        return data
