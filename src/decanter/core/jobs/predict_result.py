...
from decanter.core.jobs.task import PredictTask, PredictTSTask, GPPredictTask
...
class GPPredictResult(Job):
    """GPPredictResult manage to get the results from predictions.

    Handle the execution of predict task in order to predict model on
    Decanter GP server; Stores the predict results in PredictResult's attributes.
    ...
    def __init__(self, predict_input, name=None):
        super().__init__(
            jobs=[predict_input.data, predict_input.experiment],
            task=GPPredictTask(predict_input, name=name),
            name=gen_id(self.__class__.__name__, name))
        self.accessor = None
        self.schema = None
        self.originSchema = None
    ...
    def show(self):
        ...
        if self.is_success():
            pred_txt = check_response(
                self.gp_service.get_table_csv_by_id(self.id)).text
        else:
            logger.error('[%s] fail', self.__class__.__name__)
        return pred_txt
    ...
    def show_df(self):
        ...
        if self.is_success():
            pred_csv = check_response(
                self.gp_service.get_table_csv_by_id(self.id))
            pred_csv = pred_csv.content.decode('utf-8')
            pred_df = pd.read_csv(io.StringIO(pred_csv))
        else:
            logger.error('[%s] fail', self.__class__.__name__)
