...
from decanter.gp.extra import GPStatus
from decanter.gp.gp_api import GPAPI, worker
...
class Context:
    ...
    def get_jobs_status(sort_by_status=False, status=None):
        ...
        if status:
            if any(stat not in GPStatus.ALL_STATUS for stat in status):
