...
exp_ts.id, exp_ts.status, exp_ts.result = (
    EXP_ID,
    CoreStatus.DONE,
    core_results["train_ts"],
)
...
def context_fixture():
    ...
    def mock_request_body(cond):
        return {
            "Healthy": {"status": 400},
            "AuthFailed": {"json": {}, "status": 405},
            "RequestException": {"json": {}, "status": 405},
        }.get(cond, "error")

    def mocked_context(cond):
        params = mock_request_body(cond)
        responses.add(responses.DELETE, TEST_HEALTHY_URL, **params)
