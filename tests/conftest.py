...
from corex import Context, CoreXAPI, Model, MultiModel,\
                  TrainInput, TrainTSInput,\
                  PredictInput, PredictTSInput
from corex.corex_vars import CoreXStatus
from corex.jobs import DataUpload, Experiment, ExperimentTS
...
data = DataUpload()
data.id, data.status, data.result = DATA_ID, CoreXStatus.DONE, corex_results['upload']

fail_data = DataUpload()
fail_data.status = CoreXStatus.FAIL
...
exp.id, exp.status, exp.result = EXP_ID, CoreXStatus.DONE, corex_results['train']
exp.best_model = best_model

fail_exp = Experiment(train_input)
fail_exp.status = CoreXStatus.FAIL

exp_ts = ExperimentTS(train_ts_input)
exp_ts.id, exp_ts.status, exp_ts.result = EXP_ID, CoreXStatus.DONE, corex_results['train_ts']
exp_ts.best_model = best_multi_model

fail_exp_ts = ExperimentTS(train_ts_input)
fail_exp_ts.status = CoreXStatus.FAIL
...
def corex():
    """Return class:`CoreXAPI <CoreXAPI>` for unittests."""
    return CoreXAPI()
...
def mock_test_responses():
    ...
    def mocked_responses(task, status=None, task_result='default'):
        if task_result == 'default':
            task_result = corex_results[task]
